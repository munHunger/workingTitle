...
public class Entity implements Paintable
{
	...
	protected void validateSize()
	{
		...
		size.setX(lowX);
		size.setY(lowY);
		size.setWidth(Math.max(1, highX - lowX));
		size.setHeight(Math.max(1, highY - lowY));
	}
	...
	public void paint(Graphics2D g2d, float xOffset, float yOffset, boolean displace)
	{
		...
		BufferedImage image = new BufferedImage((int) ((size.getWidth() + 1) * zoom),
				(int) ((size.getHeight() + 1) * zoom), BufferedImage.TYPE_4BYTE_ABGR);
